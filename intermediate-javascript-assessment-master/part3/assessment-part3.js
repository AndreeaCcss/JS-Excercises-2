...
function callBinding(magicAnimals, updateAnimal, id){
    for(i=0; i<magicAnimals.length;i++){
        if(magicAnimals[i].id == id){
            let result = updateAnimal.call(magicAnimals[i], 'Trogdor');
            return result;
        };
    };
};
...
// with the context of the animal, and the array ['being majestic', 'eating rainbows'] 
// as a parameter.
// return the result of your updateAnimal invocation
...
function applyBinding(magicAnimals, updateAnimal, id){
    for(i = 0; i<magicAnimals.length; i++){
        if(magicAnimals[i].id == id){
            array = ['being majestic', 'eating rainbows']
            let result = updateAnimal.apply(magicAnimals[i], array);
            return result;
        };
    };
};
...
// In your function, create a custom promise, then create a timeout with a 
// duration of 20 ms.
// The timeout function should update the variable foo (seen below) to equal 'bar'.
// After the timeout is completed, the promise should be resolved with 
// the new updated foo variable.
// NOTE: Manually invoking your function here will alter the 'foo' variable 
// before tests run, causing them to fail.
...
// Make an array of emails (array of strings) from the returned data 
// (You will need to console log or debug to figure this out),
